...
contract Voting {
	...
	struct voter {
		...
		uint[] voteTypes;
		uint voteCount; // Array to keep track of votes per candidate.
	 ...
	 */
	function voteForCandidate(bytes32 candidate, uint votesInTokens, uint isNotFake) {
		...
		// voterInfo[msg.sender].voteTypes.push(isNotFake);
	...
	function incrementTokenValue(uint value) {
		voterInfo[msg.sender].tokensBought = value;
	}
